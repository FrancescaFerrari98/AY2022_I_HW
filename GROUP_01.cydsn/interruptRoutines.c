...
CY_ISR (Custom_ISR_ADC){
    ...
    /* Ciclo per campionare alternativamente il sensore di temperatura ed il Light Dependent Resistor:
       ...
       questo ciclo prosegue fino a quando il contatore non supera 2*conv_shift.
       Se conv_shift=0, quindi il numero di campioni non è stato ancora impostato il campionamento non avviene*/
    if(counter%2==0 && counter<2*conv_shift && conv_shift !=0){
        //Selezione del canale a cui connettere l'ADC. Se è 0 si campiona il sensore di 
        //temperatura, se è 1 l'LDR
    ...
    }else if(counter%2==1 && counter<2*conv_shift && conv_shift !=0){
        AMux_Select(counter%2);
        ADC_DelSig_StartConvert();
        value_digit = ADC_DelSig_Read32(); 
        if(value_digit <0) value_digit=0;
    ...
    }else if(counter>=2*conv_shift && conv_shift !=0){
        // quando il contatore supera 2*conv_shift sono stati ottenuti 5 campioni per entrambi i sensori.
        // Si calcolano quindi le medie (sui 5 campioni ottenuti) per entrambi i sensori.
        avg_temp=sum_temp/conv_shift;
        avg_ldr=sum_ldr/conv_shift;
